...
def start(msg):
    bot.send_message(
        chat_id=msg.from_user.id, text=f"Добро пожаловать в лучший в мире калькулятор"
    )
    bot.send_message(
        chat_id=msg.from_user.id, text=f"Текущее число {print_number(number)}"
    )
...
def interface(msg: telebot.types.Message):
    ...
    if check_number(msg.text) != "Ошибка":
        ...
        next_message = bot.send_message(
            chat_id=msg.from_user.id, text="Число обновлено"
        )
        bot.send_message(
            chat_id=msg.from_user.id, text=f"Текущее число {print_number(number)}"
        )
        bot.send_message(
            chat_id=msg.from_user.id, text=f"Введите команду или новое число"
        )
        ...
        next_message = bot.send_message(
            chat_id=msg.from_user.id, text="Введите следующее число"
        )
...
def calculations(msg: telebot.types.Message):
    ...
    if check_number(msg.text) != "Ошибка":
        ...
        next_message = bot.send_message(
            chat_id=msg.from_user.id, text="Расчёт произведён"
        )
        bot.send_message(
            chat_id=msg.from_user.id, text=f"Результат равен {print_number(number)}"
        )
        bot.send_message(
            chat_id=msg.from_user.id, text=f"Введите команду или новое число"
        )
