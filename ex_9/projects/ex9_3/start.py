...
def start(msg):
    bot.send_message(
        chat_id=msg.from_user.id, text="Вас приветствует телефонный справочник"
    )
    try:
        file = open("database.csv", "r", encoding="latin-1")
        file.close()
    except:
        with open("database.csv", "w", newline="", encoding="latin-1") as csvfile:
    ...
    main_rmk.add(
        view_note_interface_button,
        add_note_button,
        export_interface_button,
        import_interface_button,
    )
    bot.send_message(
        chat_id=msg.from_user.id, text=f"Выберите операцию", reply_markup=main_rmk
    )
...
def export_interface(msg):
    ...
    next_message = bot.send_message(
        chat_id=msg.from_user.id, text=f"Выберите формат", reply_markup=export_rmk
    )
...
def choose_export(msg):
    ...
    if msg.text == "csv":
        ...
        create_csv()
        bot.send_document(
            chat_id=msg.from_user.id, document=open("export/data.csv", "rb")
        )
    elif msg.text == "xml":
        create_xml()
        bot.send_document(
            chat_id=msg.from_user.id, document=open("export/data.xml", "rb")
        )
    elif msg.text == "txt":
        create_txt()
        bot.send_document(
            chat_id=msg.from_user.id, document=open("export/data.txt", "rb")
        )
    start
...
def import_file(msg):
    ...
    with open(path, "wb", encoding="latin-1") as f_out:
...
def main_interface(msg):
    if msg.text == "Просмотреть справочник":
        ...
        next_message = bot.send_message(
            chat_id=msg.from_user.id, text=f"Введите фамилию:"
        )
        ...
        next_message = bot.send_message(
            chat_id=msg.from_user.id, text=f"Отправьте файл:"
        )
